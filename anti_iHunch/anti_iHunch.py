...
def trigger_warning(
    locations: list, fhp_time: float, noface_conti: int, limit: int, face_area: float
) -> tuple:
    """
...
class FHPDetection:
    ...
    def label_object(
        self, location: tuple, text: str, frame: np.ndarray, face_area: float
    ):
        """
    ...
    def start_detection(self) -> None:
        ...
        while ret_val:
            ...
            small_frame = cv2.resize(frame, (0, 0), fx=self.shrink, fy=self.shrink)
    ...
    def set_configure(self, camera: int = 0, skip: bool = False) -> None:
        ...
        lines = [
            ...
            f"Set the camera. \nPress 0 for original one, press 1 for additional one. \nNow is {camera}",
            ...
            if frame_count % self.detect_every_n_frames == 0:
                ...
                if text == lines[1] and not skip:
                    if chr(wait_key & 255) == str(camera):
                        ...
                        self.set_configure(camera=int(chr(wait_key & 255)), skip=True)
                        return
                if text == lines[2]:
